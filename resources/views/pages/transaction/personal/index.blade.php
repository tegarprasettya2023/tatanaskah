                                ...
                                <td>
                                    <div class="btn-group btn-group-sm">
                                       ...
                                       @elseif($item->jenis === 'notulen')
                                        ...
                                        <a href="{{ route('transaction.personal.surat_keputusan.edit', $item->id) }}" 
                                        class="btn btn-outline-warning me-2" title="Edit">
                                            <i class="bi bi-pencil fs-5"></i>
                                        </a>
                                        @if($item->generated_file)
                                            <a href="{{ route('transaction.personal.surat_keputusan.preview', $item->id) }}" 
                                            class="btn btn-outline-primary me-2" title="Preview PDF" target="_blank">
                                                <i class="bi bi-file-pdf fs-5"></i>
                                            </a>
                                            <a href="{{ route('transaction.personal.surat_keputusan.download', $item->id) }}" 
                                            class="btn btn-outline-success me-2" title="Download PDF" target="_blank">
                                                <i class="bi bi-download fs-5"></i>
                                            </a>
                                        @endif
                                        <form action="{{ route('transaction.personal.surat_keputusan.destroy', $item->id) }}" 
                                            method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <button class="btn btn-outline-danger" 

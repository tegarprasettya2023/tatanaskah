...
@section('content')
<x-breadcrumb :values="[__('menu.transaction.menu'), 'Surat Pribadi', 'SPO']" />

<div class="card mb-4">
    <form action="{{ route('transaction.personal.spo.store') }}" method="POST" enctype="multipart/form-data">
        @csrf

        <div class="card-header">
            <h5>Buat Standar Prosedur Operasional (SPO)</h5>
        </div>
        ...
        <div class="card-body row">
            ...
            <div class="col-md-4 mb-3">
                ...
                <select class="form-select @error('logo_kiri') is-invalid @enderror" id="logo_kiri" name="logo_kiri" required>
                    ...
                    <option value="klinik" {{ old('logo_kiri') == 'klinik' ? 'selected' : '' }}>Klinik</option>
            ...
            <div class="col-md-4 mb-3">
                ...
                <select class="form-select @error('logo_kanan') is-invalid @enderror" id="logo_kanan" name="logo_kanan" required>
                    ...
                    <option value="klinik" {{ old('logo_kanan') == 'klinik' ? 'selected' : '' }}>Klinik</option>
            ...
            @for($i = 1; $i <= 10; $i++)
                @if($i == 5)
                    ...
                    <div class="col-12 mb-3">
                        ...
                        <input type="text" class="form-control mb-2" id="label_{{ $i }}" name="label_{{ $i }}" 
                            value="{{ old('label_' . $i, 'Bagan Alir') }}" placeholder="Ubah label jika perlu">
                        ...
                        <small class="text-muted">Format: JPG, PNG, GIF, SVG, WEBP. Maks: 2MB</small>
                ...
                @elseif($i == 10)
                    ...
                    <div class="col-12 mb-3">
                        <label for="label_{{ $i }}" class="form-label">{{ $i }}. Label</label>
                        <input type="text" class="form-control mb-2" id="label_{{ $i }}" name="label_{{ $i }}" 
                            value="{{ old('label_' . $i, 'Rekaman Historis Perubahan') }}" placeholder="Ubah label jika perlu">
                            ...
                            <div class="row mb-2 historis-row">
                                <div class="col-md-2">
                                    <input type="text" class="form-control form-control-sm" 
                                        name="rekaman_historis[0][no]" placeholder="No." 
                                        value="{{ old('rekaman_historis.0.no') }}">
                                </div>
                                <div class="col-md-4">
                                    <input type="text" class="form-control form-control-sm" 
                                        name="rekaman_historis[0][yang_diubah]" placeholder="Yang Diubah" 
                                        value="{{ old('rekaman_historis.0.yang_diubah') }}">
                                </div>
                                <div class="col-md-4">
                                    <input type="text" class="form-control form-control-sm" 
                                        name="rekaman_historis[0][isi_perubahan]" placeholder="Isi Perubahan" 
                                        value="{{ old('rekaman_historis.0.isi_perubahan') }}">
                                </div>
                                <div class="col-md-2">
                                    <input type="text" class="form-control form-control-sm" 
                                        name="rekaman_historis[0][tanggal_berlaku]" placeholder="Tgl Berlaku" 
                                        value="{{ old('rekaman_historis.0.tanggal_berlaku') }}">
                                </div>
                            </div>
                        </div>
                ...
                @else
                    ...
                    <div class="col-12 mb-3">
                        <label for="label_{{ $i }}" class="form-label">{{ $i }}. Label</label>
                        <input type="text" class="form-control mb-2" id="label_{{ $i }}" name="label_{{ $i }}" 
                            value="{{ old('label_' . $i, ['Pengertian', 'Tujuan', 'Kebijakan', 'Prosedur', '', 'Hal-hal Yang Perlu Diperhatikan', 'Unit terkait', 'Dokumen terkait', 'Referensi'][$i-1] ?? '') }}" 
                        ...
                        <textarea class="form-control @error('content_' . $i) is-invalid @enderror" 
                            ...
                            placeholder="Masukkan isi konten...">{{ old('content_' . $i) }}</textarea>
                        @error('content_' . $i)
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
        ...
        <div class="card-footer">
            <button type="submit" class="btn btn-primary">
                <i class="bi bi-save"></i> Simpan SPO
            </button>

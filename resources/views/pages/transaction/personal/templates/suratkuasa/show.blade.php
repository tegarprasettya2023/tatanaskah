...
@section('content')
<x-breadcrumb :values="[__('menu.transaction.menu'), 'Surat Pribadi', 'Surat Kuasa', 'Detail']" />

<div class="row">
    <div class="col-lg-8">
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5><i class="bi bi-file-earmark-text"></i> Detail Surat Kuasa</h5>
                <div class="btn-group">
                    <a href="{{ route('transaction.personal.suratkuasa.preview', $data->id) }}" 
                       class="btn btn-success btn-sm" target="_blank">
                        <i class="bi bi-eye"></i> Preview PDF
                    </a>
                    <a href="{{ route('transaction.personal.suratkuasa.download', $data->id) }}" 
                       class="btn btn-primary btn-sm" target="_blank">
                        <i class="bi bi-download"></i> Download PDF
                    </a>
                    <a href="{{ route('transaction.personal.suratkuasa.edit', $data->id) }}" 
            ...
            <div class="card-body">
                ...
                <div class="row mb-4">
                    <div class="col-12">
                        <h6 class="text-primary border-bottom pb-2 mb-3">
                            <i class="bi bi-person"></i> Pemberi Kuasa
                        </h6>
                    </div>
                    <div class="col-md-6 mb-2">
                        <strong>Nama:</strong><br>
                        <span class="text-muted">{{ $data->nama_pemberi ?? '-' }}</span>
                    </div>
                    <div class="col-md-6 mb-2">
                        <strong>NIP:</strong><br>
                        <span class="text-muted">{{ $data->nip_pemberi ?? '-' }}</span>
                    </div>
                    <div class="col-md-6 mb-2">
                        <strong>Jabatan:</strong><br>
                        <span class="text-muted">{{ $data->jabatan_pemberi ?? '-' }}</span>
                    </div>
                    <div class="col-md-6 mb-2">
                        <strong>Alamat:</strong><br>
                        <span class="text-muted">{{ $data->alamat_pemberi ?? '-' }}</span>
                    </div>
                </div>
            </div>
        </div>
